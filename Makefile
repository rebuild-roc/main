...
#  做。
#
#  以下所提及的 GitHub，其基礎網址為 <https://github.com>，I2P 暗網上有一個
#  Gitlab 網站，其網址為 <http://git.idk.i2p>，只能通過 I2P 連接，也可以通過
#  TorBrowser 連接
...
#      		make push-forced
#    (2) 從 GitHub 和 idk 等代碼託管網站的遠端倉庫下拉某些分支到本地倉庫，則
#      在命令行中執行
#		make pull
#      或
#		make fetch
#    (3) 如果本地倉庫在 GitHub 或 idk 等代碼託管網站上並不存在，
#      可以用
#    		make create
#      把本地倉庫上推到上述網站，以後就可以用 make pull 和 make push 把本地倉庫
#      和上述代碼託管網站上的遠端倉庫同步更新了。不過，在執行 make create 前，
#      需要先登錄到 GitHub 上建立一個空的倉庫 (而 idk 上則不需要預先創建空
#      倉庫)；假設空倉庫的名稱為 <repo>，還需要完成下面配置步驟中的 (1) 才可以
#      執行上述的 make create 。
#
#      在進行上述操作前，要先在您的本地倉庫中的配置好遠端倉庫及對應的上游分支。
#  下面的例子是配置 GitHub 遠端倉庫以及其中的上游分支的方法，對於 idk 的遠端
#  倉庫也是類似的。
