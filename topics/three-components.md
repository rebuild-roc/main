...
現在，對於一個賬戶而言，設它某次資金轉出或證券賣出的強度爲 $s$,
...
而只是舉個滿足這些要求的例子：
$$f(s,t) = S(s) T(t),$$
其中 $S$ 和 $T$ 都是只依賴於一個變量，$S$ 是嚴格遞增的上凸函數，並且在 $s$
接近於 1 或更大時 $S$ 的值也接近於 1；而 $T$ 則是嚴格遞減的函數，並且在 $t$
很大的時候趨近於零。例如，可以取
$$S(s) = \frac{2}{\pi} \arctan\left(\frac{s}{s_0}\right),$$
