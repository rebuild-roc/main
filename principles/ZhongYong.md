...
由於具體尋找（或逼近）$m$ 的方法很多，這裏我們只介紹正規理論中的三分法和黃金分割法。
此外，我們還要介紹第三種算法，與三分法和黃金分割法的出發點差別很大。

### 1. 三分法

給定了某個區間 $[a, b]$ 上的單峯函數 $f$, 三分法 (ternary search) 就是尋找 $f$
...
所以令 $l = a$ 和 $r = b$,
隨後進入以下循環過程。
1. 把區間 $[l, r]$ 分爲三段，也就是說，要取兩個數 $x_1$ 和 $x_2$ 使之把區間
  $[l, r]$ 分爲長度相等的三段 $[l, x_1]$
  和 $[x_1, x_2]$
  以及 $[x_2, r]$；
...
搜尋區間的長度會縮短爲循環前的 $1/\varphi$, 這個值大約爲 $0.618$,
比 $2/3$ 小，
...
3. 將上述中點 $x$ 用作決策選擇之後，接下來觀察其後果，根據後果評估這個 $x$
  ...
  這就代表步驟 2 中的決策太猛，那就把 $x$
  用作下一輪決策的 $r$,
  然後回到步驟 1 再來一次決策；如果無法判斷究竟是 $x < m$
